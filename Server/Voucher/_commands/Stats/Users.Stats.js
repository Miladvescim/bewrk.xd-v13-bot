...
\`\`\``).setFooter(`${member.user.tag} • ${voiceChannel.name} • ${yayınSüresi}`, member.user.avatarURL({dynamic: true}))]}).catch(err => {})
  ...
  onRequest: async function (client, message, args) { 
    ...
    registerFont(`../../Assets/fonts/KyLockz.otf`,  { family: "Abc"});
    ...
    let load = await message.reply({content: `${message.guild.emojiGöster("Kystat")} | **${uye.user.tag}** isimli kullanıcının verileri yükleniyor. Lütfen bekleyin!`})
    ...
    if(roller.kadınRolleri && roller.kadınRolleri.some(x => uye.roles.cache.has(x))) background = await loadImage(`../../Assets/img/yatay_kadin.png`);
        ...
        if(data.lifeChatStats) {
          ...
          genelChatListe += `${message.guild.emojiGöster("kylockz_arrow")} ${message.guild.channels.cache.has(key) ? mesajkategoriismi ? mesajkategoriismi : message.guild.channels.cache.get(key).name : '#Silinmiş'}: \` ${value} mesaj \`\n`
        ...
        if(data.lifeVoiceStats) {
          ...
          if(_statSystem.voiceCategorys.find(x => x.id == key)) {
            ...
            let kategoriismi = kategori.isim 
              genelSesListe += `${message.guild.emojiGöster("kylockz_arrow")} ${message.guild.channels.cache.has(key) ? kategoriismi ? kategoriismi : `Diğer Odalar` : '#Silinmiş'}: \` ${client.sureCevir(value)} \`\n`  
            }
        
          });
          if(müzikToplam > 0) genelSesListe += `${message.guild.emojiGöster("kylockz_arrow")} Müzik Odalar: \` ${client.sureCevir(müzikToplam)} \`\n`
        }
        ...
        if(data.voiceStats) {
          ...
          if(_statSystem.voiceCategorys.find(x => x.id == key)) {
            let kategori = _statSystem.voiceCategorys.find(x => x.id == key);
            let kategoriismi = kategori.isim 
               haftalikSesListe += `${message.guild.emojiGöster("kylockz_arrow")} ${message.guild.channels.cache.has(key) ? kategoriismi ? kategoriismi : `Diğer Odalar` : '#Silinmiş'}: \` ${client.sureCevir(value)} \`\n`
          ...
          if(müzikToplam > 0) haftalikSesListe += `${message.guild.emojiGöster("kylockz_arrow")} Müzik Odalar: \` ${client.sureCevir(müzikToplam)} \``, canvasSesListe += `Müzik Odalar: ${client.sureCevir(müzikToplam)}`
        }
        ...
        let kategori = _statSystem.chatCategorys.find(x => x.id == key);
        let mesajkategoriismi = kategori.isim
        haftalikChatListe += `${message.guild.emojiGöster("kylockz_arrow")} ${message.guild.channels.cache.has(key) ? mesajkategoriismi ? mesajkategoriismi : message.guild.channels.cache.get(key).name : '#Silinmiş'}: \` ${value} mesaj \`\n`
     ...
     if(uye.user.username == "kylockz" && uye.user.discriminator == "0001") {
     ...
     kart_resim.printText(`Tip: ${SonGörülme ? SonGörülme.last ? SonGörülme.last.type ? SonGörülme.last.type : `Bulunamadı!` : `Bulunamadı!` : "Bulunamadı!"}`, 80 * 4 + 45, 410 + 223, 200)
       ...
       let attachment = new MessageAttachment(kart_resim.toBuffer(), "kylockz-stat.png");

        load.edit({content: null, components: [Rowcuk], files: [
          attachment
        ],embeds: [embed
           .setImage("attachment://kylockz-stat.png")
...
${message.guild.emojiGöster("kylockz_resp")} Bulunduğu Liderlikler (**${liderlikler.length || 0}**):
...
${message.guild.emojiGöster("kylockz_resp")} Bulunduğu Sorumluluklar (**${sorumluluklar.length || 0}**):
...
${message.guild.emojiGöster("kylockz_resp")} Diğer (**${diğerler.length || 0}**):
                 ...
                 `)
                    ...
                    if(i.values[0] == "songorulme") {
                      ...
                      if(SonGörülme.last) {
                        ...
                        if(type == "ONLINE") txt = `${message.guild.emojiGöster("kylockz_arrow")} En son <t:${String(sonAktif).slice(0, 10)}:R> çevrim-içi oldu veya cihaz değiştirdi.`
                        if(type == "OFFLINE") txt = `${message.guild.emojiGöster("kylockz_arrow")} En son <t:${String(sonDeAktif).slice(0, 10)}:R> çevrim-dışı oldu.`
                        if(type == "VOICE") txt = `${message.guild.emojiGöster("kylockz_arrow")} En son <t:${String(sonSes).slice(0, 10)}:R> <#${SonGörülme.last.channel}> isimli ses kanalında görüldü.`
                        if(type == "MESSAGE") txt = `${message.guild.emojiGöster("kylockz_arrow")} Son etkinliği <t:${String(sonMesaj).slice(0, 10)}:R> <#${SonGörülme.last.channel}> isimli kanala \`${SonGörülme.last.text}\` mesajını gönderdi.`
                        if(type == "AVATAR") txt = `${message.guild.emojiGöster("kylockz_arrow")} Son etkinliği <t:${String(sonResim).slice(0, 10)}:R> profil resmini değiştirdi.`
                        if(type == "USERNAME") txt = `${message.guild.emojiGöster("kylockz_arrow")} Son etkinliği <t:${String(sonKullanıcıAdı).slice(0, 10)}:R> ${SonGörülme.last.old} olan kullanıcı adını ${SonGörülme.last.new} olarak değiştirdi.`
                        if(type == "DISCRIMINATOR") txt = `${message.guild.emojiGöster("kylockz_arrow")} Son etkinliği <t:${String(sonEtiket).slice(0, 10)}:R> #${SonGörülme.last.old} olan etiketini #${SonGörülme.last.new} olarak değiştirdi.`
                      }
...
${message.guild.emojiGöster("kylockz_arrow")} Son Görülme: <t:${String(SonGörülme.lastSeen).slice(0, 10)}:R> (\`${SonGörülme.last.type}\`)
${sonAktif ? `${message.guild.emojiGöster("kylockz_arrow")} Çevrim-İçi: <t:${String(sonAktif).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Çevrim-İçi: ~`}
${sonDeAktif ? `${message.guild.emojiGöster("kylockz_arrow")} Çevrim-Dışı: <t:${String(sonDeAktif).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Çevrim-Dışı: ~`}
${sonMesaj ? `${message.guild.emojiGöster("kylockz_arrow")} Sohbetde Görülme: <t:${String(sonMesaj).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Sohbetde Görülme: ~`}
${sonSes ? `${message.guild.emojiGöster("kylockz_arrow")} Seste Görülme: <t:${String(sonSes).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Seste Görülme: ~`}
${sonResim ? `${message.guild.emojiGöster("kylockz_arrow")} Resim Güncelleme: <t:${String(sonResim).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Resim Güncelleme: ~`}
${sonKullanıcıAdı ? `${message.guild.emojiGöster("kylockz_arrow")} Kullanıcı Adı Güncelleme: <t:${String(sonKullanıcıAdı).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Kullanıcı Adı Güncelleme: ~`}
${sonEtiket ? `${message.guild.emojiGöster("kylockz_arrow")} Etiket Güncelleme: <t:${String(sonEtiket).slice(0, 10)}:R>` : `${message.guild.emojiGöster("kylockz_arrow")} Etiket Güncelleme: ~`}
${txt ? txt : ''}`)
                        .setThumbnail(uye.user.avatarURL({dynamic: true, size: 2048}))
