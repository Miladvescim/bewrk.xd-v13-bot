...
module.exports = {
    ...
    Kullanim: "removebalance <Altın/Para> <@KyLockz/ID> <Miktar>",
    Aciklama: "",
    Kategori: "eco",
    Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!args[0]) return message.reply(`${cevaplar.prefix} Lütfen hangi birimden geri alacağını belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}removebalance <Altın/Para> <@KyLockz/ID> <Miktar>\` )`).then(x => {
    ...
    return message.reply(`${cevaplar.prefix} Lütfen hangi birimden geri alacağını belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}removebalance <Altın/Para> <@KyLockz/ID> <Miktar>\` )`).then(x => {
        message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
        setTimeout(() => {
            x.delete()
        }, 7500);
