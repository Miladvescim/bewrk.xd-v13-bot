...
module.exports = {
    ...
    Kullanim: "transfer <Altın/Para> <@KyLockz/ID> <Miktar>",
    Aciklama: "",
    Kategori: "eco",
    Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    let Coin = 0
    if(!args[0]) return message.reply(`${cevaplar.prefix} Lütfen hangi birimden göndereceğini belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}transfer <Altın/Para> <@KyLockz/ID> <Miktar>\` )`).then(x => {
    ...
    return message.reply(`${cevaplar.prefix} Lütfen hangi birimden göndereceğini belirt. (Örn: \`${sistem.botSettings.Prefixs[0]}transfer <Altın/Para> <@KyLockz/ID> <Miktar>\` )`).then(x => {
        message.react(message.guild.emojiGöster(emojiler.Iptal) ? message.guild.emojiGöster(emojiler.Iptal).id : undefined)
        setTimeout(() => {
            x.delete()
        }, 7500);
