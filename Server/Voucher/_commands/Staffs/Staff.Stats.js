...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let barSystem = [
      ...
      {id: "diger", başlamaBar: "kylockz_baslangicbar", doluBar: "kylockz_dolubar", doluBitişBar: "kylockz_dolubitisbar", boşBitişBar:"kylockz_bosbitis", boşBar: "kylockz_bosbar", başlangıçBar: "kylockz_bosbaslama"},
    ]
    ...
    if(görevBul) {
      ...
      if(data.taskVoiceStats) {
        data.taskVoiceStats.forEach((value, key) => {
            genelses += value
            if(key == kanallar.publicKategorisi) public += Number(client.kylockzSaatYap(value))
            if(key == kanallar.streamerKategorisi) public += Number(client.kylockzSaatYap(value))
            if(key == kanallar.registerKategorisi) register += Number(client.kylockzSaatYap(value))
        });
      };
        ...
        if(button.customId === "gorev_bilgisi") {
          ...
          if(görevBul.publicVoice >= 1) {
           ...
           if(görevBul.AllVoice >= 1) {
             embedcik.addFields({ name: `${görevBul.AllVoice} Saat Tüm Ses Kanallarında Takıl!`, value: `${Number(client.kylockzSaatYap(genelses)) >= görevBul.AllVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Mesaj) } ${yönetimBar(Number(client.kylockzSaatYap(genelses)) ? Number(client.kylockzSaatYap(genelses)) : 0, görevBul.AllVoice, 5, Number(client.kylockzSaatYap(genelses)))} \`${Number(client.kylockzSaatYap(genelses)) >= görevBul.AllVoice ? `Tamamlandı!`: `${Number(client.kylockzSaatYap(genelses))} saat / ${görevBul.AllVoice} saat`}\``})
           }
