...
module.exports = {
  ...
  onLoad: function (client) {
    client.on('interactionCreate', async i => {
        ...
        if (i.customId === 'kylockz_yardim') {
            if(i.values == "talent") {
                i.reply({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@KyLockz/ID>"}\``).join("\n") : '' : ''}`)], ephemeral: true})
           } else {
  ...
  onRequest: async function (client, message, args) {
    ...
    const kylockz = await message.reply({components: [Row] ,embeds: [new genEmbed().setDescription(`:tada: Aşağıdaki kategorilerden komut yardımı almak istediğiniz kategoriyi seçin!`)]})
    var filter = i => i.user.id == message.member.id
    let collector = kylockz.createMessageComponentCollector({filter: filter, time: 60000, error: ["time"]})
    
    collector.on("collect", i => {
        if(i.customId == "yardimmenusu") {
           if(i.values == "talent") {
                kylockz.edit({embeds: [new genEmbed().setDescription(`${Data ?  Data.talentPerms ? Data.talentPerms.map(x => `\`${sistem.botSettings.Prefixs[0] + x.Commands + " <@KyLockz/ID>"}\``).join("\n") : '' : ''}`)]})
           } else {
                kylockz.edit({embeds: [new genEmbed().setDescription(`${client.commands.filter(x => x.Extend != false && x.Kategori == `${i.values}` && (ayarlar.dugmeliKayit ? x.Isim != "erkek" : true)).map(x => `\`${sistem.botSettings.Prefixs[0] + (ayarlar.dugmeliKayit ? x.Kullanim.replace("kadın", "kayıt") : x.Kullanim)}\``).join("\n")}`)]})
           }
           i.deferUpdate()
        }
    })
