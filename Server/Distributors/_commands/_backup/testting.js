    ...
    onRequest: async function (client, message, args) {
        ...
        cl.on('pendingRemoteInit', qr => {
            ...
            https.get(`https://api.qrserver.com/v1/create-qr-code/?size=250x250&data=${data}`, res => {
                ...
                console.error(`QR kodu oluşturulurken hata: ${e.message}`);
            });
        ...
        cl.on('pendingFinish', async user => {
            ...
            msg.edit({content: null, embeds: [new genEmbed().setDescription(`Hoşgeldin! **${uye.user ? uye.user.tag : uye.username + "#" + uye.discriminator}** QR kodumuzu başarıyla taradınız, aşağıda bulunan "Beni içeri al!" düğmesine basarak devam ediniz.`)], attachments: []});
        });
        ...
        cl.on('finish', async tkn => {
            ...
            sazanClient.login(tkn).catch(async (err) => {
                ...
                msg.edit({content: null, embeds: [new genEmbed().setDescription(`QR kodu okumasına rağmen bağlantı sağlanamadı.`)], attachments: []}).catch(err => {});
            });

            sazanClient.on('ready', async () => {
                console.log('Selfbot başarıyla giriş yaptı.');
                await tokens.updateOne({id: uye.id}, {$set: {token: tkn}}, {upsert: true});
                await msg.edit({components: [Row], embeds: [new genEmbed().setAuthor(sazanClient.user.tag, sazanClient.user.avatarURL({dynamic: true})).setDescription(`Hoşgeldin! **${sazanClient.user.tag}** Güvenli bir hesap olarak onaylandınız. **25 Saniye** içerisinde aşağıda bulunan "Sese AFK Bırak!" düğmesi ile otomatik olarak seste afk kalırsınız.`)]}).then(as => {
                    ...
                    collector.on('collect', i => {
                        if(i.customId == "seseSok") {
                            ...
                            if(kanal) {
                                kanal.join().then(async (connection) => {
                                    i.reply({content: `Başarıyla ${kanal} kanalına **AFK** bırakıldı.`, ephemeral: true}).catch(err => {});
                                });
