  ...
  module.exports = {
    ...
    onRequest: async function (client, message, args) {
      ...
      const buttonSatir = new MessageActionRow().addComponents(
        ...
        .setOptions(
          {label: "Gerçekleşen Ayarlar", emoji: {id:"1194730154220597289"}, description: "Yapılan (gerçekleşen) tüm ayar listesi bulunmaktadır.", value: "ayarlar"},
          {label: "Değiştirilebilir Ayarlar", emoji: {id: "1192517340684877954"}, description: "Düzenlenmesi münkün olan ayarların listesi bulunmaktadır.", value: "ayarlistesi"},
          {label: "Güvenlik Sistemi", emoji: {id:"1194724574789644459"}, description: "Sunucu üzerinde ki güvenliğin kontrolünü sağlayan ayarlar bulunmaktadır.", value: "ayarlar2"},
          {label: "Bot Düzenleme", emoji: {id: "1192412950489731102"}, description: "Belirtilen botun profil resmi, isim ve hakkındasını düzenlenebilir.", value: "botsettings"},
          {label: "Yedekleme", emoji: {id:"1194732905969221782"}, description: "Anlık sunucunun verilerini ve içeriklerini yedeklemektedir.", value: "ayarlar1"},
      ...
      let bilgilendirme = [
        `Sunucu üzerinde yapılmış tüm ayarlarınızı listelemek mi istiyorsun? "Gerçekleşen Ayarlar" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucu için gereken tüm ayarları listelemek mi istiyorsun? "Değiştirilebilir Ayarlar" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucunun tüm rol, kanal, ayarlar ve emojilerinin yedeklemesini almak mı istiyorsun? "Yedekleme" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucuna ekip geldi ve ayrı göstermek mi istiyorsun? "Ekip Sistemi" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucunun ve botunuzun log kanalları yok mu? "Log Kurulumu" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Botunun tüm emojileriniz eksik mi? "Emoji Kurulumu" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Botunun süresi mi bitti? "Süre & Ödeme İşlemleri" veya "Web Yönetim Paneli" kategorilerine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `İstatistikleri temizlemek mi istiyorsun? "İstatistikleri Sıfırla" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Botlarını yeniden başlatmak mı istiyorsun? "Yeniden Başlat" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Herşeyi sıfırlamak mı istiyorsun (Sil Baştan)? "Tüm Ayarları Sıfırla" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucun için rol verip/alan bir komut mu eklemek istiyorsun? "Özel Komut Sistemi" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Sunucunun güvenliği için gereken tüm sistemi görmek mi istiyorsun? "Güvenlik Sistemi" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Bot resimlerini, isimlerini ve hakkındasını düzenlemek mi istiyorsun? "Bot Düzenleme" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Rol seçim menüsü veya düğme sistemi kurmak mı istiyorsun? "Rol & Seçenek Kurulumu" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Herşeyin kolay olmasını ister misin? "Web Yönetim Paneli" kategorisine göz atabilirsin. ${message.guild.emojiGöster(emojiler.kylockzdev)}`,
        `Herşeyi dilediğiniz gibi düzenleyebileceksiniz, artık sizin elinizde tüm ayarlarınız. ${message.guild.emojiGöster(emojiler.kylockzdev)}`
      ]
...
> ${bilgilendirme[Math.floor(Math.random()*bilgilendirme.length)]}`)], components: [buttonSatir]}).then(async (x) => {
                ...
                collector.on('collect', async i => {
                  ...
                  if(i.values[0] === 'webpanel') {
                    if(i.user.id != "817463869487185980") return await i.reply({content: `${cevaplar.prefix} Sunucu sahibi veya da bot sahibi site üzerinden erişim sağlayabilir..`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})
                    await i.reply({content: `Sizi yönlendiriyorum...`, ephemeral: true})
                  }
                  if(i.values[0] === 'sure') {
                    if(i.user.id != "817463869487185980") return await i.reply({content: `${cevaplar.prefix} Sistemde profilin girili olmadığı için süre ve ödeme işlemlerine ulaşamazsın.`, ephemeral: true}),
                    message.react(message.guild.emojiGöster(emojiler.Onay) ? message.guild.emojiGöster(emojiler.Onay).id : undefined),
                    x.delete().catch(err => {})

                    await i.reply({embeds:[new genEmbed().setColor("DARK_AQUA").setDescription(`${message.guild.emojiGöster(emojiler.Icon)} **Merhaba!** Sayın. KYLOCKZ (\`ceo@kylockz.com\`), ${message.guild.name} (\`ÖMÜR BOYU - LİFE END\`) Sunucusunun ödeme, süre ve paket işlemleri için aşağıda bilgilendirmesi yapılmış ufak kategorilerden kontrol edebilir veya düğmelerden işlem yaparak yada websitesi üzerinden güncelleme gibi işlemleri yapabilirsiniz.`)
                    .addField(`\` ••❯ \` Ödeme Durumu`, `\` • \` **Ödeme Tipi**: Sanal Pos (\`VISA\`)\n\` • \` **Ödeme Tarihi**: 16.12.2021 17:42\n\` • \` **Onay Tarihi**: 16.12.2021 17:43`, true)
                    .addField(`\` ••❯ \` Lisans Bilgisi`, `\` • \` Lisans Tipi: \`Business\`\n\` • \` Lisans Numarası: \`KYLOCKZ (Owner)\``, true)], components: [ödeme],ephemeral: true})
                  ...
                  if(i.values[0] === "ayarlar1") {
                    ...
                    x.delete().catch(err => {})
                     await i.reply({files: [{
                      ...
                      name: `KY-${secretOluştur(5)}.kylockz`,
...
Size verilen dosyayı indirerek \`.setup\` komutunu kullanırken ek olarak eklendiğinde bu aldığınız yedeklemeyi tekrardan kurabileceksiniz.`, ephemeral: true})
                    ...
                    if (i.values[0] === 'ayarlar') {
                      ...
                      x.delete().catch(err => {})
                      let ayarlist = await message.channel.send({embeds: [new genEmbed().setColor("WHITE").setDescription(`${message.guild.emojiGöster(emojiler.Onay)} Aşağıda ki ayarlar kategorisinden hangi yapılan ayar listesini görüntülemek istediğini seçerek görüntüleyebilirsiniz.`)], components: [Rows, RowsTWO]}).then(async (msg) => {
