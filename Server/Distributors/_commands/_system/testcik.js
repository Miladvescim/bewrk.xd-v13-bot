...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let yetkiler = data.filter(m => {
        ...
        return uyeToplam2 <= ms("12h") && message.guild.members.cache.has(m.userID) && m.userID != "817463869487185980" && !uye.permissions.has("ADMINISTRATOR") && !uye.permissions.has("MANAGE_ROLES") && roller.Yetkiler.some(x => uye.roles.cache.has(x))
    })
    let mesaj = ''
