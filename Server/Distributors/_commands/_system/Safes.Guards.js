...
module.exports = {
    Isim: "güvenlik",
    Komut: ["yarram", "guard-settings","guards","kylockzguard","guard"],
    Kullanim: "",
    Aciklama: "",
    Kategori: "-",
    Extend: true,
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
    .setFooter(`Bir üyeyi beyaz listeye ekleyip/çıkartmak için ${global.sistem.botSettings.Prefixs[0]}guard <@KyLockz/ID> komutunu kullanabilirsiniz.`)
    if(!uye) {
        message.channel.send({embeds: [anaMenu], components: [oneRows]}).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "guard_manager") {
                ...
                let kylockzRow = new MessageActionRow().addComponents([
                    new MessageButton()
                ...
                msg.delete().catch(err => {})
                message.channel.send({embeds: [anaMenu], components: [kylockzRow, backMenu]}).then(async (gmsg) => {
                    var filter = (i) => i.user.id == message.member.id
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        if(i.customId == "guardState") {
                            ...
                            if(guardSettings.guildProtection) {
                                kylockzRow.components[2].setDisabled(true)
                                kylockzRow.components[3].setDisabled(true)
                                kylockzRow.components[1].setDisabled(true)
                                kylockzRow.components[4].setDisabled(true)
                                kylockzRow.components[0].setStyle("DANGER").setLabel(`Güvenliği Aç`)
                                ...
                                kylockzRow.components[1].setDisabled(false)
                                if(guardSettings.limit) kylockzRow.components[2].setDisabled(false)
                                kylockzRow.components[3].setDisabled(false)
                               if(ayarlar.SMS) kylockzRow.components[4].setDisabled(false)
                                kylockzRow.components[0].setStyle("SECONDARY").setLabel(`Güvenliği Kapat`)
                        ...
                        if(i.customId == "limitState") {
                            ...
                            if(guardSettings.limit) {
                                kylockzRow.components[2].setDisabled(true)
                                kylockzRow.components[1].setStyle("DANGER").setLabel(`Limiti Aç`)
                                await GUARD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, { $set: {"limit": false }}, {upsert: true})
                            } else if(!guardSettings.limit) {
                                kylockzRow.components[2].setDisabled(false)
                                kylockzRow.components[1].setStyle("SECONDARY").setLabel(`Limiti Kapat`)
...
Olarak ayarlandı.`)], components: [row]}).then(async (kylockz) => {
    ...
    collector.on('end', i => kylockz.delete().catch(err => {}))
    collector.on('collect', async (i) => {
        kylockz.delete().catch(err => {})
        let kom = client.commands.find(x => x.Isim == "güvenlik")
        kom.onRequest(client, message, args)
        i.deferUpdate().catch(err => {})
                        ...
                        if(i.customId == "smsState") {
                            ...
                            if(guildSettings) ayarlar = guildSettings.Ayarlar
                            if(ayarlar.SMS) {
                                kylockzRow.components[3].setStyle("DANGER").setLabel(`SMS/MMS Aç`)
                                kylockzRow.components[4].setDisabled(true).setStyle("SUCCESS").setLabel(`SMS/MMS Gerekli`)
                                ...
                                kylockzRow.components[4].setDisabled(false).setLabel(`WhatsApp/Telegram Aç`).setStyle("DANGER")
                                kylockzRow.components[3].setStyle("SECONDARY").setLabel(`SMS/MMS Kapat`)
                        ...
                        if(i.customId == "wpState") {
                            ...
                            if(ayarlar.WHATSAPP | ayarlar.TELEGRAM) {
                                kylockzRow.components[4].setStyle("DANGER").setLabel(`WhatsApp/Telegram Aç`)
                                await GUILD_SETTINGS.updateOne({guildID: sistem.SERVER.ID}, { $set: {"Ayarlar.WHATSAPP": false, "Ayarlar.TELEGRAM": false }}, {upsert: true})
                                
                            } else if(!ayarlar.WHATSAPP || !ayarlar.TELEGRAM) {
                                kylockzRow.components[4].setStyle("SECONDARY").setLabel(`WhatsApp/Telegram Kapat`)
...
**Son Güvenlik Kayıtları**
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
                        gmsg.edit({embeds: [anaMenu], components: [kylockzRow, backMenu]}).catch(err => {})  
                        i.deferUpdate().catch(err => {})
                    })
                })
              }
