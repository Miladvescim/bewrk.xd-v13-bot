...
message.reply({content: `**Bahis Devam Ediyor!** ${message.guild.emojiGöster("kylockz_cf")}
...
Belirlenen Miktar: \` ${Miktar.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} ${ayarlar.serverName} Parası \``}).then(msg => {
        setTimeout(async () => {
            ...
            let cc = Miktar*2;
            msg.edit({content: `**Bahis Bitti!** ${rnd == 1 ? message.guild.emojiGöster("kylockz_cfwin") : message.guild.emojiGöster("kylockz_cflose")}
