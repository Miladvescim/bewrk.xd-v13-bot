...
${limitController.sort((a, b) => b.date - a.date).map((x, index) => `${index+1}. | ${x.proc} | ${tarihsel(x.date)}`).join("\n")}
        ...
        async queryManage(oldData, newData) {
            ...
            if(sunucuData) {              
                ...
                if(oldData === sunucuData.etiketKanalı) {
                    await guildSettings.updateOne({guildID: sistem.SERVER.ID}, {$set: {"Ayarlar.etiketKanalı": newData}})
                }
            ...
            this.login(token).then(kylockz => {
                ...
                this.on("ready", async () => { 
                    ...
                    if(this.botName == "Sync" || this.botName == "Guard_I") {
                        let kanal = this.channels.cache.get(global.ayarlar ? global.ayarlar.botSesKanalı : "1185119806052126740")
